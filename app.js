  ...
  if (words.length) {
    words.forEach((word) => {
      html += `<div class="flex items-center w-full text-lg font-medium border-b-2 divide-slate-200 py-2">
        <p class="font-medium text-lg mr-[153px]">${word.id * 1 + 1}</p>
        <p id="enWord" class="mr-[149px]">${word.en}</p>
        <p id="uzWord" class="mr-32">${word.uz}</p>
        <div class="flex gap-3">
          <button id="check">
            <i class="fa-solid fa-circle-check" style="color: #06cf02" onclick="check(${word})"></i>
          </button>
          <button id="delete">
...
function check(checkword) {
  //   words.filter((word) => word.id == checkword.id)
  words = words.filter((word) => word.id != checkword.id);
  checkWords.push(words.filter((word) => word.id == checkword.id));
  console.log(words);
}
